...
a menor rota que visita um conjunto de cidades e retorna à cidade original, 
visitando cada cidade exatamente uma vez.

***Funcionamento:
O programa começa mostrando um menu ao usuário. 
O usuário pode inserir novos pontos de entrega, novos itens, novos caminhões, 
associar itens a pontos de entrega, associar pontos de entrega a caminhões e, 
finalmente, realizar entregas.

Ao realizar entregas, o programa utiliza a abordagem do "vizinho mais próximo" para determinar a rota 
que cada caminhão deve seguir. Esta abordagem é gulosa porque, a cada passo, o caminhão se desloca para 
o ponto de entrega mais próximo do seu local atual, sem considerar o quadro geral. 
Isso pode não fornecer a rota mais curta globalmente, mas é uma heurística útil para encontrar 
rapidamente uma solução aproximada.
